...
@export var physical_dice_type_scenes : Dictionary = {
	DiceRoll.DiceType.BASE : preload("res://Scenes/3D/DiceRoll/Dices/BasePhysicalDice.tscn"),
	DiceRoll.DiceType.SKILL : preload("res://Scenes/3D/DiceRoll/Dices/SkillPhysicalDice.tscn"),
	DiceRoll.DiceType.GEAR : preload("res://Scenes/3D/DiceRoll/Dices/GearPhysicalDice.tscn"),
}
...
func _instantiate_dices():
	...
	var circular_layout = Layouts3D.get_circular_layout(throw_origin.global_transform,
		dice_roll_result.result.size(),
		throw_layout)
...
func _play_time_scale_animation():
	...
	_time_scale_animation.tween_method(_handle_time_scale_animation_final_slowdown, 1.0, 0.01, final_slowdown_duration)
	# Finally stop them
...
func _animate_dices_results():
	...
	while(unpositioned_dices != 0):
		...
		if(unpositioned_dices > result_max_row_elements):
			...
			row_positions = Layouts3D.get_horizontal_layout(next_row_transform,
				result_max_row_elements,
				current_horizontal_layout)
			# Progress variables
			unpositioned_dices -= result_max_row_elements
			next_row_transform.origin += -result_origin.global_transform.basis.y * result_layout.spacing
		# Uncomplete row
		else:
	...
	for i in _physical_dices.size():
		...
		position_tween.tween_property(_physical_dices[i], 
				"position", 
				dice_result_positions[i], 
		...
		look_at_camera_transform = look_at_camera_transform.looking_at(camera.position, Vector3.UP, true)
		...
		rotation_tween.tween_property(_physical_dices[i], 
				"quaternion",
