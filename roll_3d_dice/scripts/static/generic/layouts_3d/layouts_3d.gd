...
static func get_circular_layout(transform : Transform3D, element_count : int, circular_layout_parameters : CircularLayout3DParameters):
	...
	for i in element_count:
		...
		if(level != 0):
			...
			var offset = transform.basis.x * cos(current_split_angle) + transform.basis.y * sin(current_split_angle)
...
static func get_horizontal_layout(transform : Transform3D, element_count : int, horizontal_layout_parameters : HorizontalLayout3DParameters):
	# Initialize return value
	var positions = []
	
	# Compute parameters
	var total_length = element_count * horizontal_layout_parameters.element_width + horizontal_layout_parameters.spacing * (element_count - 1)
	...
	for i in element_count:
		...
		position += (horizontal_layout_parameters.element_width + horizontal_layout_parameters.spacing) * transform.basis.x * i
